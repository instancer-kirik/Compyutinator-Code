...
class FileManager:
    ...
    def new_document(self):
        ...
        self.cccore.auratext_window.tab_widget.addTab(editor, "Untitled")

    def open_file(self, path=None):
        if path is None:
            path, _ = QFileDialog.getOpenFileName(self.cccore.auratext_window, "Open File")
        if path:
            self._open_file(path)

    def _open_file(self, path):
        if self.cccore.auratext_window and hasattr(self.cccore.auratext_window, 'tab_widget'):
            # Check if the file is already open
            for i in range(self.cccore.auratext_window.tab_widget.count()):
                editor = self.cccore.auratext_window.tab_widget.widget(i)
                if editor.file_path == path:
                    self.cccore.auratext_window.tab_widget.setCurrentIndex(i)
                    return
            ...
            try:
                ...
                if new_editor:
                    index = self.cccore.auratext_window.tab_widget.addTab(new_editor, os.path.basename(path))
                    self.cccore.auratext_window.tab_widget.setCurrentIndex(index)
    ...
    def save_file(self, editor=None):
        ...
        try:
            ...
            index = self.cccore.auratext_window.tab_widget.indexOf(editor)
            self.cccore.auratext_window.tab_widget.setTabText(index, os.path.basename(editor.file_path))
        except Exception as e:
    ...
    def save_file_as(self, editor=None):
        ...
        path, _ = QFileDialog.getSaveFileName(self.cccore.auratext_window, "Save File As")
        if not path:
            return False
    ...
    def close_tab(self, index):
        editor = self.cccore.auratext_window.tab_widget.widget(index)
        if editor.isModified():
            reply = QMessageBox.question(self.cccore.auratext_window, "Save Changes?",
            ...
            if reply == QMessageBox.StandardButton.Save:
                ...
                return False
        self.cccore.auratext_window.tab_widget.removeTab(index)
        return True
    
    def create_fileset(self):
        name, ok = QInputDialog.getText(self.cccore.auratext_window, "Create Fileset", "Enter fileset name:")
        if ok and name:
            files, _ = QFileDialog.getOpenFileNames(self.cccore.auratext_window, "Select Files for Fileset")
            if files:
                if self.cccore.vault_manager.create_fileset(name, files):
                    QMessageBox.information(self.cccore.auratext_window, "Success", f"Fileset '{name}' created successfully.")
                    self.update_fileset_dropdowns()
                else:
                    QMessageBox.warning(self.cccore.auratext_window, "Error", f"Fileset '{name}' already exists.")

    def open_fileset(self):
        filesets = self.cccore.vault_manager.get_all_filesets()
        if not filesets:
            QMessageBox.information(self.cccore.auratext_window, "No Filesets", "No filesets available. Create one first.")
            return

        fileset_name, ok = QInputDialog.getItem(self.cccore.auratext_window, "Open Fileset", "Select a fileset:", filesets, 0, False)
    ...
    def update_fileset(self):
        ...
        fileset_name, ok = QInputDialog.getItem(self.cccore.auratext_window, "Update Fileset", "Select a fileset to update:", filesets, 0, False)
        if ok and fileset_name:
            files, _ = QFileDialog.getOpenFileNames(self.cccore.auratext_window, "Select Files for Fileset")
            if files:
                if self.cccore.vault_manager.update_fileset(fileset_name, files):
                    QMessageBox.information(self.cccore.auratext_window, "Success", f"Fileset '{fileset_name}' updated successfully.")
                    self.update_fileset_dropdowns()
                else:
                    QMessageBox.warning(self.cccore.auratext_window, "Error", f"Failed to update fileset '{fileset_name}'.")

    def delete_fileset(self):
        filesets = self.cccore.vault_manager.get_all_filesets()
        if not filesets:
            QMessageBox.information(self.cccore.auratext_window, "No Filesets", "No filesets available to delete.")
            return

        fileset_name, ok = QInputDialog.getItem(self.cccore.auratext_window, "Delete Fileset", "Select a fileset to delete:", filesets, 0, False)
        if ok and fileset_name:
            confirm = QMessageBox.question(self.cccore.auratext_window, "Confirm Deletion", f"Are you sure you want to delete the fileset '{fileset_name}'?",
                                           QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            if confirm == QMessageBox.StandardButton.Yes:
                if self.cccore.vault_manager.delete_fileset(fileset_name):
                    QMessageBox.information(self.cccore.auratext_window, "Success", f"Fileset '{fileset_name}' deleted successfully.")
                    self.update_fileset_dropdowns()
                else:
                    QMessageBox.warning(self.cccore.auratext_window, "Error", f"Failed to delete fileset '{fileset_name}'.")
    ...
    def open_workspace(self):
        ...
        if not workspaces:
            QMessageBox.information(self.cccore.auratext_window, "No Workspaces", "No workspaces available. Create one first.")
            return

        workspace_name, ok = QInputDialog.getItem(self.cccore.auratext_window, "Open Workspace", "Select a workspace:", workspaces, 0, False)
        if ok and workspace_name:
            if self.cccore.workspace_manager.switch_workspace(workspace_name):
                ...
                QMessageBox.information(self.cccore.auratext_window, "Success", f"Workspace '{workspace_name}' opened successfully.")
            else:
                QMessageBox.warning(self.cccore.auratext_window, "Error", f"Failed to open workspace '{workspace_name}'.")

    def create_workspace(self):
        name, ok = QInputDialog.getText(self.cccore.auratext_window, "Create Workspace", "Enter workspace name:")
        if ok and name:
            vault_path = QFileDialog.getExistingDirectory(self.cccore.auratext_window, "Select Vault Directory")
            if vault_path:
                filesets = self.cccore.vault_manager.get_all_filesets()
                fileset_name, ok = QInputDialog.getItem(self.cccore.auratext_window, "Create Workspace", "Select a fileset (optional):", 
                ...
                if ok:
                    ...
                    if self.cccore.workspace_manager.create_workspace(name, vault_path, fileset_name):
                        QMessageBox.information(self.cccore.auratext_window, "Success", f"Workspace '{name}' created successfully.")
                    else:
                        QMessageBox.warning(self.cccore.auratext_window, "Error", f"Workspace '{name}' already exists.")
    ...
    def create_new_folder(self):
        ...
        folder_name, ok = QInputDialog.getText(self.cccore.auratext_window, "Create New Folder", "Enter folder name:")
        if ok and folder_name:
            new_folder_path = os.path.join(current_path, folder_name)
            try:
                os.makedirs(new_folder_path)
                QMessageBox.information(self.cccore.auratext_window, "Success", f"Folder '{folder_name}' created successfully.")
                self.cccore.widget_manager.file_explorer.refresh()
            except OSError as e:
                QMessageBox.warning(self.cccore.auratext_window, "Error", f"Failed to create folder: {str(e)}")

    def rename_selected_item(self):
        selected_item = self.cccore.widget_manager.file_explorer.get_selected_item()
        if selected_item:
            old_name = os.path.basename(selected_item)
            new_name, ok = QInputDialog.getText(self.cccore.auratext_window, "Rename Item", "Enter new name:", text=old_name)
            if ok and new_name:
                old_path = selected_item
                new_path = os.path.join(os.path.dirname(old_path), new_name)
                try:
                    os.rename(old_path, new_path)
                    QMessageBox.information(self.cccore.auratext_window, "Success", f"Item renamed from '{old_name}' to '{new_name}'.")
                    self.cccore.widget_manager.file_explorer.refresh()
                except OSError as e:
                    QMessageBox.warning(self.cccore.auratext_window, "Error", f"Failed to rename item: {str(e)}")
        else:
            QMessageBox.warning(self.cccore.auratext_window, "Error", "No item selected for renaming.")

    def delete_selected_item(self):
        selected_item = self.cccore.widget_manager.file_explorer.get_selected_item()
        if selected_item:
            item_name = os.path.basename(selected_item)
            confirm = QMessageBox.question(self.cccore.auratext_window, "Confirm Deletion", 
            ...
            if confirm == QMessageBox.StandardButton.Yes:
                try:
                    ...
                    QMessageBox.information(self.cccore.auratext_window, "Success", f"'{item_name}' deleted successfully.")
                    self.cccore.widget_manager.file_explorer.refresh()
                except OSError as e:
                    QMessageBox.warning(self.cccore.auratext_window, "Error", f"Failed to delete item: {str(e)}")
        else:
            QMessageBox.warning(self.cccore.auratext_window, "Error", "No item selected for deletion.")
