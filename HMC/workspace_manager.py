...
class WorkspaceManager:
    def __init__(self, vault_manager, cccore):
        ...
        self.config_file = os.path.join(self.vault_manager.get_current_vault_path() or tempfile.gettempdir(), "workspace_config.json")
    ...
    def load_workspaces(self):
        if self.vault_manager.vault_path:
            workspace_configs = [f for f in os.listdir(self.vault_manager.vault_path) if f.startswith('.workspace_') and f.endswith('.json')]
            for config in workspace_configs:
                name = config[10:-5]  # Remove '.workspace_' prefix and '.json' suffix
                self.workspaces[name] = Workspace(name, self.vault_manager.vault_path)
        else:
    ...
    def create_workspace(self, name):
        ...
        if sanitized_name not in self.workspaces:
            vault_path = self.vault_manager.vault_path or tempfile.gettempdir()
